...
from pyphoon.app.custom_layers import Scale
...
def DenseNet(nb_dense_block=4, growth_rate=32, nb_filter=64, reduction=0.0,
    ...
    x = Convolution2D(nb_filter, (7, 7), strides=(2, 2), name='conv1',
                      use_bias=False)(x)
    ...
    x = BatchNormalization(epsilon=eps, axis=concat_axis, name='conv'+str(
        final_stage)+'_blk_bn')(x)
...
def conv_block(x, stage, branch, nb_filter, dropout_rate=None,
               weight_decay=1e-4):
    '''Apply BatchNorm, Relu, bottleneck 1x1 Conv2D, 3x3 Conv2D, and option
    dropout
        # Arguments
            x: input tensor
