...
seq_list = [(int(seq_no), 'train') for seq_no in ids_train] + [(int(seq_no), 'test') for seq_no in ids_test]
...
de.generate_images_besttrack_chunks(seq_list, chunk_size=4 * 1024 ** 3,
                                    output_dir=output_dir,
                                    preprocess_algorithm=preprocessor.apply, display=True)
